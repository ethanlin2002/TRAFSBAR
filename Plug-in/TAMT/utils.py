...
def load_model_skip_head2(model, ckpt_path, strict=False):
    ckpt = torch.load(ckpt_path, map_location="cpu", weights_only=False)
    state_dict = ckpt.get("module") or ckpt.get("state_dict") or ckpt.get("state") or ckpt
    ...
    # ✅ remove incompatible head2 weights (and bias)
    for k in list(state_dict.keys()):
        if k.startswith("head2."):
            state_dict.pop(k)

    missing, unexpected = model.load_state_dict(state_dict, strict=strict)
    print("[load_model] missing:", [m for m in missing if m.startswith("head2.")], " ...")
    print("[load_model] unexpected:", [u for u in unexpected if u.startswith("head2.")], " ...")
    return model