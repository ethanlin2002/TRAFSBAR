...
parser.add_argument('--dataset', default='mini_imagenet', choices=['Rareact2','Rareact','d2iving48', 'k400','ucf101','hmdb51','tiered_imagenet', 'SSv2Full','SSv2Small','diving48','finesports','multisports'])
...
parser.add_argument('--n_query', default=6, type=int, help='number of unlabeled data in each class during meta validation')
...
parser.add_argument('--traj_lam', default=1.0, type=float)
...
novel_datamgr = SetDataManager(
    params.data_path,
    params.image_size,
    n_query=params.n_query,
    n_episode=params.test_n_episode,
...
use_yolo= str('yolo' in params.traj_root)

with open ('%s/test.txt'%params.checkpoint_dir, 'w',encoding='utf-8') as f:
      f.write('dataset:%s, shot=%d, lam=%.1f, yolo=%s\n'%(params.dataset, params.n_shot, params.traj_lam, use_yolo))
      f.write('%d Test Acc = %4.2f%% +- %4.2f%%\n'% (iter_num, acc_mean, 1.96 * acc_std / np.sqrt(iter_num)))
      f.write('%d test mean acc = %4.2f%%' % (params.test_task_nums, acc_all_task_mean))