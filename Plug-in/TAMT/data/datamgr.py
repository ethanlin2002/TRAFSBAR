...
class SetDataManager(DataManager):
    def __init__(self, data_path, image_size, n_way, n_support, n_query, n_episode, json_read=False, params=None):
    ...
    def get_data_loader(self, data_file, aug):  # parameters that would change on train/val set
        transform = self.trans_loader.get_composed_transform(aug)
        if self.json_read:
            dataset = SetDataset_JSON(self.image_size, self.data_path, data_file, self.batch_size, transform, args=self.params)
        else:
        ...
        #print("SetDataManager self.params.traj_root =", getattr(self.params, "traj_root", None))
        return data_loader
