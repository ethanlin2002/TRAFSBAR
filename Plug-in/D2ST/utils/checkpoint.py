...
def get_path_to_checkpoint(path_to_job, epoch):
    ...
    """
    name = "checkpoint_epoch_{:02d}.pyth".format(epoch)
...
def get_last_checkpoint(path_to_job):
    ...
    Priority:
      1. checkpoint_last.pyth
      2. latest checkpoint by name
    """
    d = get_checkpoint_dir(path_to_job)
    names = os.listdir(d) if os.path.exists(d) else []
    names = [f for f in names if "checkpoint" in f and f.endswith(".pyth")]
    assert len(names), "No checkpoints found in '{}'.".format(d)

    # 優先使用固定檔名 checkpoint_last.pyth
    if "checkpoint_last.pyth" in names:
        return os.path.join(d, "checkpoint_last.pyth")
...
def save_last_checkpoint(path_to_job, model, optimizer, epoch, cfg, model_bucket=None):
    """
    Save checkpoint_last.pyth.
    This file is overwritten every epoch and used for auto resume.
    """
    if not du.is_master_proc(cfg.NUM_GPUS * cfg.NUM_SHARDS):
        return

    checkpoint_dir = get_checkpoint_dir(path_to_job)
    if not os.path.exists(checkpoint_dir):
        os.makedirs(checkpoint_dir)

    path_to_checkpoint = os.path.join(checkpoint_dir, "checkpoint_last.pyth")

    ms = model.module if hasattr(model, "module") else model

    checkpoint = {
        "epoch": epoch,
        "model_state": ms.state_dict(),
        "optimizer_state": optimizer.state_dict(),
        "cfg": cfg.dump(),
    }

    torch.save(checkpoint, path_to_checkpoint)
    logger.info("Saved checkpoint_last.")


def get_best_checkpoint_path(path_to_job):
    """
    Get path to checkpoint_best.pyth.
    """
    return os.path.join(get_checkpoint_dir(path_to_job), "checkpoint_best.pyth")

def load_best_metric(path_to_job):
    """
    Load best metric from checkpoint_best.pyth.
    If checkpoint_best does not exist, return -inf.
    """
    path_to_best = get_best_checkpoint_path(path_to_job)

    if not os.path.exists(path_to_best):
        return float("-inf")

    with open(path_to_best, "rb") as f:
        checkpoint = torch.load(f, map_location="cpu")

    if "best_metric" in checkpoint:
        best_metric = checkpoint["best_metric"]
    elif "best_top1_acc" in checkpoint:
        best_metric = checkpoint["best_top1_acc"]
    else:
        best_metric = float("-inf")

    logger.info("Loaded best_metric: {:.4f}".format(best_metric))
    return best_metric
...
def save_best_checkpoint(
    ...
    """
    Save checkpoint_best.pyth if current metric is better.

    Args:
        epoch: zero-based finished epoch.
        cur_metric: current eval top1 accuracy.
        best_metric: previous best top1 accuracy.

    Returns:
        updated best_metric.
    """
    if cur_metric is None:
        logger.info("Skip saving best checkpoint because cur_metric is None.")
        return best_metric

    if cur_metric <= best_metric:
        logger.info(
            "Best checkpoint not updated. cur_metric={:.4f}, best_metric={:.4f}".format(
    ...
    checkpoint = {
        ...
        "best_top1_acc": cur_metric,
        "model_state": normalized_sd,
        "optimizer_state": optimizer.state_dict(),
        "cfg": cfg.dump(),
    }

    path_to_checkpoint = get_best_checkpoint_path(path_to_job)

    with open(path_to_checkpoint, "wb") as f:
        torch.save(checkpoint, f)

    logger.info(
        "Saved checkpoint_best. | best_epoch={} | best_top1_acc={:.4f}".format(
            epoch + 1,
            cur_metric,
        )
    )
...
def load_test_checkpoint(cfg, model, model_bucket=None):
    """
    ...
    if cfg.TEST.CHECKPOINT_FILE_PATH != "" and cfg.TEST.CHECKPOINT_FILE_PATH is not None:
        ...
        best_checkpoint = os.path.join(checkpoint_dir, "checkpoint_best.pyth")

        if os.path.exists(best_checkpoint):
            checkpoint_path = best_checkpoint
            logger.info("Load from the best checkpoint file: {}".format(checkpoint_path))
        else:
            checkpoint_path = get_last_checkpoint(cfg.OUTPUT_DIR)
            logger.info("Load from the last checkpoint file: {}".format(checkpoint_path))
...
def load_train_checkpoint(cfg, model, optimizer, model_bucket=None):
    """
    ...
    if cfg.TRAIN.AUTO_RESUME and has_checkpoint(cfg.OUTPUT_DIR):
        ...
        checkpoint_epoch = load_checkpoint(
            cfg, last_checkpoint, model, cfg.NUM_GPUS * cfg.NUM_SHARDS > 1, optimizer,
